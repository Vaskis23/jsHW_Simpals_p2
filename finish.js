...
//использу. метод массива find(), который выполняет перебор элементов массива posts и возвращает первый элемент, для которого функция-аргумент возвращает true это позволит найтиобъект с нужным postId
function findPostById(postId, posts) {
  // метода find(), используем стрелочную функцию которая принимает объект из массива и проверяет равено ли свойство postId значению postId  переданному в качестве аргумента функции.
